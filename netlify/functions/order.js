...
export default async (request) => {
  ...
  return json({
    ...
    message: fulfillment === "doordash"
      ...
      : "Order received! Send your payment now using the button below, then we'll confirm and start cooking."
  });
};
...
Confirm payment received before cooking.`;
...
async function sendEmail({ subject, text }) {
  ...
  const from = process.env.ORDER_EMAIL_FROM || "orders@sweetandsalao.com";
  if (!key || !to) return;
